...
export const extractT12Data = (data: any[] | string, fileType: string): T12Data => {
  ...
  if (fileType === 'pdf') {
    ...
    // Look for common T12 patterns
    lines.forEach((line: string) => {
      if (line.toLowerCase().includes('total income') || line.toLowerCase().includes('gross income')) {
        const match = line.match(/[\d,]+\.?\d*/);
        if (match) result.totalIncome = parseFloat(match[0].replace(/,/g, ''));
      }
      
      if (line.toLowerCase().includes('total expense') || line.toLowerCase().includes('operating expense')) {
        const match = line.match(/[\d,]+\.?\d*/);
        if (match) result.totalExpenses = parseFloat(match[0].replace(/,/g, ''));
      }
      
      if (line.toLowerCase().includes('net operating income') || line.toLowerCase().includes('noi')) {
        const match = line.match(/[\d,]+\.?\d*/);
        if (match) result.netOperatingIncome = parseFloat(match[0].replace(/,/g, ''));
      }
    });
  } else {
